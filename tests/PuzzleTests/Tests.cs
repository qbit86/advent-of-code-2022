...
{
    [Theory]
    [InlineData("sample-input.txt", 15L)]
    [InlineData("input.txt", 9651L)]
    ...
    [Theory]
    [InlineData("input.txt", 14451L)]
    internal async Task PartOne_LessThan(string inputPath, long upperBound)
    {
        using StreamReader input = new(inputPath, Encoding.UTF8);
        var actual = await Puzzles.PartOne.SolveAsync(input).ConfigureAwait(false);
    ...
    [Theory]
    [InlineData("sample-input.txt", 12L)]
    [InlineData("input.txt", 10560L)]
